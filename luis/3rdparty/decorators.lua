...
function Slice9Decorator:draw(this)
    ...
    love.graphics.draw(self.image, love.graphics.newQuad(iw - self.right, 0, self.right, self.top, iw, ih),
        x + w - self.right, y)
    love.graphics.draw(self.image, love.graphics.newQuad(0, ih - self.bottom, self.left, self.bottom, iw, ih), x,
        y + h - self.bottom)
    love.graphics.draw(self.image,
        love.graphics.newQuad(iw - self.right, ih - self.bottom, self.right, self.bottom, iw, ih), x + w - self.right,
        y + h - self.bottom)

    -- Draw edges
    love.graphics.draw(self.image, love.graphics.newQuad(self.left, 0, cw, self.top, iw, ih), x + self.left, y, 0,
        (w - self.left - self.right) / cw, 1)
    love.graphics.draw(self.image, love.graphics.newQuad(self.left, ih - self.bottom, cw, self.bottom, iw, ih),
        x + self.left, y + h - self.bottom, 0, (w - self.left - self.right) / cw, 1)
    love.graphics.draw(self.image, love.graphics.newQuad(0, self.top, self.left, ch, iw, ih), x, y + self.top, 0, 1,
        (h - self.top - self.bottom) / ch)
    love.graphics.draw(self.image, love.graphics.newQuad(iw - self.right, self.top, self.right, ch, iw, ih),
        x + w - self.right, y + self.top, 0, 1, (h - self.top - self.bottom) / ch)

    -- Draw center
    love.graphics.draw(self.image, love.graphics.newQuad(self.left, self.top, cw, ch, iw, ih), x + self.left,
        y + self.top, 0, (w - self.left - self.right) / cw, (h - self.top - self.bottom) / ch)
